...
func Plugin(ctx context.Context) *plugin.Plugin {
	p := &plugin.Plugin{
		...
		TableMap: map[string]*plugin.Table{
			...
			"ovh_cloud_project_usage_current":  tableOvhCloudProjectUsageCurrent(),
			"ovh_cloud_project_usage_forecast": tableOvhCloudProjectUsageForecast(),
			"ovh_cloud_project_usage_history":  tableOvhCloudProjectUsageHistory(),
			"ovh_cloud_project_usage_plans":    tableOvhCloudProjectUsagePlans(),
			...
			"ovh_dedicated_server":             tableOvhDedicatedServer(ctx),
			...
			"ovh_savings_plan_subscribed":      tableOvhSavingsPlanSubscribed(),
		},
	}
	return p
}
